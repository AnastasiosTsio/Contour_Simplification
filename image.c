...
void set_pixel_image_ameliore(Image *I, int x, int y, Pixel v)
{
	if (x<1 || x>I->L || y<1 || y>I->H)
		return;
	I->tab[INDICE_PIXEL(*I,x,y)] = v;
...
Image negatif_image(Image I)
{
	...
	UINT h=I.H;
	Image Aneg;
	Aneg=creer_image(l,h);
	for (int i=1; i<=h; i++) {
		for (int j =1; j<=l; j++){
			if (I.tab[j-1+l*(i-1)] == 0){
				// printf("C'est blanc avec i=%d et j=%d\n",i,j); 
				set_pixel_image(Aneg,j,i,NOIR);}
		}
	}
	return Aneg;
}
...
Image calculer_masque_image(Image I) {
	...
	for (int i = 1; i < I.L+1; i++) {
		for (int j = 1; j < I.H+1; j++) {
			if(j-2<0){
				if( I.tab[i-1+I.L*(j-1)] == 1){
					set_pixel_image(Masque, i ,j ,1);}
					}
			else if (I.tab[i-1+I.L*(j-2)] == 0 && I.tab[i-1+I.L*(j-1)] == 1){
				set_pixel_image(Masque, i ,j ,1);
			}
		}
	}
